#decompose(MyData$Td[1:731],type = "additive") #Decompose a time series into seasonal, trend and irregular components using moving averages
...
res4_6 <- reconstruct(mod4, sel.period = 350, show.legend = F)
...
lines(x[1:500], res4_8$series$y.r[1:500], col="gold",type="l")
...
lines(x[501:730],res4_5_1$mean[1:230],col="green",type="l")
...
for(i in 1:values){
  ...
  V_reference <- fd.estimate(e$y,methods = method, trim = T)$fd
  ...
  for(j in 1:length(Sm_guesses)){
    ...
    new_V_reference <- fd.estimate(new_Sm$e,methods = method, trim = T)$fd
